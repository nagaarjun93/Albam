...
import { Heart, Plus, Sparkles, Film, Image as ImageIcon, Lock } from 'lucide-react';
      ...
      <div className="max-w-[1920px] mx-auto px-3 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-2 sm:gap-4">
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="font-serif text-base sm:text-2xl font-bold bg-gradient-to-r from-rose-600 via-pink-600 to-rose-500 bg-clip-text text-transparent">
                Our Memories
              </span>
              <Sparkles className="w-3.5 h-3.5 text-amber-400 hidden sm:block" />
            </div>
            <p className="text-[10px] sm:text-xs text-rose-500/80 font-medium hidden sm:block">
              Naga Arjun & Wife • Forever & Always
            </p>
          </div>
        </div>

        {/* Live Counters - Responsive */}
        {stats && (
          <div className="hidden lg:flex items-center gap-2 bg-rose-50/70 border border-rose-100 rounded-full px-4 py-1.5 text-xs text-rose-800">
            <button 
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {onLock && (
            <button
              onClick={onLock}
              className="p-2 sm:px-3 sm:py-2 rounded-full border border-rose-200 bg-white hover:bg-rose-50 text-rose-600 text-xs font-medium flex items-center gap-1.5 shadow-sm transition-all"
              title="Lock album with PIN"
            >
